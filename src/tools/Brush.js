...
export default class Brush extends Tool {
    ...
    mouseDownHandler(e) {
        ...
        this.ctx.beginPath()
         if(e.targetTouches){
            this.ctx.moveTo(e.targetTouches[0].pageX - e.target.offsetLeft, e.targetTouches[0].pageY - e.target.offsetTop)
        }
    ...
    mouseMoveHandler(e) {
        if (this.mouseDown) {
            if(e.targetTouches){
                this.draw(e.targetTouches[0].pageX - e.target.offsetLeft, e.targetTouches[0].pageY - e.target.offsetTop)
            }
